...
class Contestmodel extends CI_Model {
	...
	function getContestFreezeId($id)
	{
		$q = "SELECT contest_freeze FROM pc_contest WHERE contest_id=".$id;
		$qr = $this->db->query($q);
	...
	function updateContestDetail($cname, $cdesc, $cstart, $cfreeze, $cend, $cpenalty)
	{
		$id = $this->session->userdata('contestid');
		$data = array (
			'contest_name' => $cname,
	...
	function checkContestPassword($contestid, $contestpass)
	{
		$q = '
	...
	function comboAvailableContest()
	{
		$now = now();
	...
	function getParticipantNumberContest($id)
	{
	...
	function isThisMyProblem($problemid)
	{
	...
	function showFreezeRank($contestid)
	{
	...
	function showParsialRankPublic($contestid)
	{
		$rotator = $this->Participantmodel->getProblemNumber($contestid);
