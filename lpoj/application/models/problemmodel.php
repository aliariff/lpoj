...
class Problemmodel extends CI_Model {
	...
	function getProblemContestId($problemid)
	{
	...
	function getProblemInContest($pid)
	{
	...
	function getProblemContent($problemid)
	{
	...
	function getProblemByContestId($id)
	{
		$this->db->where("CONTEST_ID",$id);
		$query = $this->db->get("pc_problem");
		return $query->result();
	}
	function getProblemContest($cid)
	{
	...
	function getNonAcceptedProblem()
	{
		$participantid = $this->session->userdata('participantid');
		$q = '
