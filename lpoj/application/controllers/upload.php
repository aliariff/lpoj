...
class Upload extends CI_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			{
				if ($_FILES["userfile"]["error"] == 1) $this->session->set_flashdata('uploaderror', "File size exceed the limit");
				else if ($_FILES["userfile"]["error"] == 2) $this->session->set_flashdata('uploaderror', "File size exceed the limit");
				else if ($_FILES["userfile"]["error"] == 3) $this->session->set_flashdata('uploaderror', "The uploaded file just partially uploaded ");
				...
				else $this->session->set_flashdata('uploaderror', "PHP Error Code : ".$_FILES["userfile"]["error"]);
