			...
			if(isset($_FILES["zip"]) && $_FILES["zip"]["tmp_name"] != ""){
				...
				if ($dir = opendir($zip_temp)){
					...
					while (($file = readdir($dir)) !== false){
						...
						if($filename[1]=="in"){
							exec("mv ".$zip_temp . $file . " /root/pclp/inputcase/testCase" . $pid . "_" . $filename[0] ); echo "mv ".$zip_temp . $file . "/root/pclp/inputcase/testCase" . $pid . "_" . $filename[0]; 
							...
							while ($line = fgets($txt)) {
								...
								for($j=0;$j<count($score);$j++){
									if(!is_numeric($score[$j]))continue;
